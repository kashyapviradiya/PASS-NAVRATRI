...
export default function EventsPage() {
  ...
  return (
    <div className="bg-navratri-bg min-h-[calc(100vh-50px)] pb-24 font-sans">
      
      {/* COMPACT SEARCH & FILTERS */}
      <div className="sticky top-[50px] md:top-[60px] z-40 bg-navratri-card/95 backdrop-blur-md border-b border-navratri-border px-3 sm:px-6 lg:px-8 py-3 mb-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-navratri-muted" />
            <input
              type="text"
              placeholder="Search events or artists"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 rounded-[12px] bg-navratri-softBg border-none text-navratri-dark font-[600] placeholder-navratri-muted focus:outline-none focus:ring-1 focus:ring-navratri-primary transition-all text-[13px]"
            />
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar">
            <div className="relative min-w-[120px] shrink-0">
              <select
                ...
                className="w-full pl-3 pr-8 py-2 rounded-[12px] bg-navratri-softBg border-none text-navratri-dark font-[600] appearance-none focus:outline-none cursor-pointer text-[13px]"
              >
                <option value="">All Cities</option>
                {CITIES.map(city => <option key={city} value={city}>{city}</option>)}
              </select>
              <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-navratri-muted pointer-events-none" />
            </div>
